...
func (rec *stubReceiver) UpdatedConsumerFor(simulationRound int, replicas int, queueLen *int64) {
	...
	for i := numToRead; i > 0; i-- {
		rec.consumerMetricsChan <- metrics.ConsumerAggregateMetric{Topic: "topic", ConsumerGroup: "stub function", Pod: "pod" /*should vary by replica*/ , Count: 1, ProcessingTime: time.Millisecond}
		(*queueLen)--
	}
}
